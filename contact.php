<?php
if (isset($_POST['envoyer'])) {
  // Extraction des variable
  extract($_POST);
  // Vérification si les vériables existent et elles ne sont pas vides
  if (
    isset($nom) && $nom != "" &&
    isset($pays) && $pays != "" &&
    isset($email) && $email != "" &&
    isset($objectif) && $objectif != "" &&
    isset($contenuM) && $contenuM != ""
  ) {

    // Envoie du mail
    $to = "irmarcellinmweze@gmail.com";

    $subject = "Vous avez reçu le message de : " . $email;

    $message = "
        <p>Vous avez un message de :" . $email . "</p>
        <p>Nom :" . $nom . "</p>
        <p>Pays :" . $pays . "</p>
        <p>Objetif :" . $objectif . "</p>
        <p>Message :" . $contenuM . "</p>

    ";

    $headers = "MIME-Version: 1.0" . "\r\n";
    $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
    $headers .= 'From: <' . $email . '>' . "\r\n";

    mail($to, $subject, $message, $headers);
    $info = 'Message envoyé !';
  } else {
    $info = 'Veuillez remplir tous les champs svp !';
  }
}


?>

<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Contact</title>
  <meta name="description" content="La communauté Letecode" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link href="https://fonts.googleapis.com/css2?family=Montserrat&display=swap" rel="stylesheet" />
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
  <link rel="shortcut icon" href="photos/logoletecode.png" />
  <link rel="stylesheet" href="styles.css" />

  <!-- Ajout Open graph -->
  <meta property="og:title" content="Contact" />
  <meta property="og:description" content="Contacter l'Admin de Letecode" />
  <meta property="og:type" content="website" />
  <meta property="og:image" content="https://marcellinmweze.github.io/tp2/photos/logoletecode.png" />
  <meta property="og:url" content="https://marcellinmweze.github.io/tp2" />
</head>

<body>
  <?php
  $page = 'contact';
  include_once('header.php');
  ?>
  <section class="section form">
    <div class="contener">
      <?php
      // Affichage du message d'erreur
      if (isset($info)) { ?>
        <div class="contenerMessage">
          <p>
            <?= $info; ?>
          </p>
        </div>
      <?php
      }

      ?>

      <h2 class="mar-t-10">Nous contacter</h2>
      <p class="mar-t-40">
        Remplissez ce formulaire pour nous laisser un message
      </p>

      <form action="contact.php" method="post">

        <div class="champs">
          <label for="">Nom
            <div>
              <input type="text" class="champtext" name="nom" />
            </div>
          </label>
          <label for="">Pays
            <div>
              <select name="pays" id="" class="champtext">
                <option selected disabled>Choisir</option>
                <option id="RDC">RDC</option>
                <option id="Zambie">Zambie</option>
                <option id="Maroc">Maroc</option>
                <option id="France">France</option>
                <option id="USA">USA</option>
                <option id="Rwanda">Rwanda</option>
                <option id="Kenya">Kenya</option>
              </select>
            </div>
          </label>
          <label for="">Adresse e-mail
            <div>
              <input type="text" class="champtext" name="email" />
            </div>
          </label>
          <label for="">Objectif
            <div>
              <input type="text" class="champtext" name="objectif" />
            </div>
          </label>
          <label for="">Message
            <div>
              <textarea name="contenuM" id="" cols="15" rows="15" class="textarea"></textarea>
            </div>
          </label>
        </div>
        <button type="submit" class="btn3" name="envoyer">Envoyer le message</button>
    </div>

    </form>
  </section>

  <section class="section boite">
    <div class="contener">
      <div class="boitelettre">
        <div class="boite-item">
          <h2 class="mar-t-10">Abonnez vous à la boîte <br />aux lettres</h2>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Mollitia,
            quae, porro nemo unde obcaecati odio aperiam labore sed numquam,
            et nam ipsam laborum. Tempora corrupti quam corporis earum alias.
            Maiores.
          </p>
        </div>

        <div class="boite-item al-center">
          <div class="texte">
            <input type="text" placeholder="Adresse email" class="txtemail" />
          </div>
          <div>
            <button class="btn2">S'abonner</button>
          </div>
        </div>
      </div>
    </div>
  </section>

  <?php

  include_once('footer.php');
  ?>
  <script src="script/menuvolet.js"></script>
</body>

</html>